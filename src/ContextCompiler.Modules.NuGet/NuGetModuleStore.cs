...
public sealed class NuGetModuleStore(IModulesLoadConfigProvider cfg, ILogger<NuGetModuleStore> logger) : INuGetModuleStore
{
    private readonly TrustPolicy _policy = new(cfg);

    public async Task<string> RestoreAsync(ModulePackageRequest req, CancellationToken ct)
    {
        ModuleSource source = cfg.Current.Sources.Single(s => string.Equals(s.Name, req.Source, StringComparison.OrdinalIgnoreCase));
    ...
    public (string authors, string? repoUrl, List<ModuleLockFile.DependencyInfo> deps, List<string> files) ReadNuspecAndDeps(string nupkgPath)
    {
        ...
        List<ModuleLockFile.DependencyInfo> deps = [];
        foreach (PackageDependencyGroup? g in nuspec.GetDependencyGroups())
        {
            foreach (PackageDependency? d in g.Packages)
            {
                ...
                deps.Add(new ModuleLockFile.DependencyInfo { Id = d.Id, Version = d.VersionRange?.OriginalString ?? "" });
            }
        }
    ...
    public string? FindModuleAssembly(string extractedRoot)
    {
