...
const fs = require("fs");
const viewsTree_1 = require("./viewsTree");
const personasTree_1 = require("./personasTree");
const commandsTree_1 = require("./commandsTree");
...
function getConfig() {
    ...
    return {
        cliPath: envCliPath.length > 0 ? envCliPath : cfg.get("path", "ctxc"),
        modulesCliPath: envCliPath.length > 0 ? envCliPath : cfg.get("modulesPath", "ctxc-modules"),
        configPath: envConfigPath.length > 0 ? envConfigPath : cfg.get("configPath", "ctxc.config.json"),
        outputDir: envOutputDir.length > 0 ? envOutputDir : cfg.get("outputDir", ".ctxc/compiled")
    };
}
function exists(p) {
    try {
...
function runCtxcCompile(output, status, cliPath, root, configPath) {
...
function runCtxcRestore(output, status, modulesCliPath, root, configPath) {
    return new Promise((resolve) => {
...
async function listViews(workspaceRoot, outputDir) {
...
async function listPersonas(workspaceRoot, outputDir) {
    const personasPath = path.join(workspaceRoot, outputDir, "personas.active.json");
    if (!exists(personasPath))
        return [];
    try {
        const parsed = JSON.parse(fs.readFileSync(personasPath, "utf-8"));
        const results = Array.isArray(parsed?.results) ? parsed.results : [];
        const ids = results
            .map((r) => (typeof r?.PersonaId === "string" ? r.PersonaId : undefined))
...
async function listCommands(workspaceRoot, outputDir) {
    ...
    try {
        ...
        const ids = commands
            .map((r) => (typeof r?.id === "string" ? r : undefined))
...
async function buildPromptFromActiveView(workspaceRoot, outputDir, activeViewId) {
    const viewsDir = path.join(workspaceRoot, outputDir, "views");
    ...
    if (exists(mdPath)) {
        ...
        try {
            ...
            if (typeof parsed === "string")
                ...
                viewContent = parsed.markdown;
            else if (typeof parsed?.content === "string")
                viewContent = parsed.content;
            else if (typeof parsed?.text === "string")
                viewContent = parsed.text;
            else
                viewContent = JSON.stringify(parsed, null, 2);
        }
        catch {
            viewContent = raw;
        }
    ...
    if (viewContent == null)
        throw new Error(`View not found: ${mdPath} or ${jsonPath}`);
    const framing = [
...
async function activate(context) {
    ...
    const inspectedPath = cfgSection.inspect("path");
    ...
    let lastRun;
    const viewsProvider = new viewsTree_1.CtxcViewsProvider(workspaceRoot, cfg.outputDir);
    vscode.window.registerTreeDataProvider("ctxcViews", viewsProvider);
    const personasProvider = new personasTree_1.CtxcPersonasProvider(workspaceRoot, cfg.outputDir);
    vscode.window.registerTreeDataProvider("ctxcPersonas", personasProvider);
    const commandsProvider = new commandsTree_1.CtxcCommandsProvider(workspaceRoot, cfg.outputDir);
    vscode.window.registerTreeDataProvider("ctxcCommands", commandsProvider);
    context.subscriptions.push(vscode.commands.registerCommand("ctxc.copyCommandFromTree", async (commandNode) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("ctxc.executeCommandFromTree", async (commandNode) => {
        const { outputDir } = getConfig();
        const commands = await listCommands(workspaceRoot, outputDir);
        if (commands.length === 0) {
            vscode.window.showInformationMessage("No commands found. Run CtxC compile first.");
        ...
        //await vscode.env.clipboard.writeText(`${command.personaId} ${command.id}:`);
        vscode.commands.executeCommand("workbench.action.chat.open", { query: `${command.personaId} ${command.id}:`,
            isPartialQuery: true });
    ...
    context.subscriptions.push(vscode.commands.registerCommand("ctxc.openPersonaFramingFromTree", async (node) => {
        if (!node?.personaId)
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand("ctxc.setActivePersonaFromTree", async (node) => {
        if (!node?.personaId)
            return;
        ...
        vscode.commands.executeCommand("workbench.action.chat.open", "load #" + promptFile + " run command role #" + node.personaId + " (en français)");
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("ctxc.setActiveViewFromTree", async (node) => {
        if (!node?.viewId)
            return;
        viewsProvider.setActiveView(node.viewId);
        vscode.window.showInformationMessage(`CtxC active view set to: ${node.viewId}`);
        const promptFile = "prompt.context.md";
        vscode.commands.executeCommand("workbench.action.chat.open", "load #" + promptFile + " run command view #" + node.fileName + " (en français)");
    }));
}
function deactivate() { }
