...
namespace ContextCompiler.Core.Pipelines
{
    public sealed class DocumentContext(ITagsBuilder tagsBuilder, IServiceProvider serviceProvider) : IDocumentContext
    {
        ...
        private readonly List<IPipelineFinding> _findings = [];
        ...
        public void SetData(IDataEnvelope envelope)
        {
            Data = Data is null ? envelope : throw new InvalidOperationException("Data already set.");
        }

        public void SetTags(IReadOnlyList<ITag> tags)
        {
            Tags = Tags is null ? tags : throw new InvalidOperationException("Tags already set.");
        }
